...
- [x] **Should videos be able to auto-post at all?** → Yes — that was always the intent, and it now works. Three stacked bugs were silently preventing it: (1) `Is Video For Posting?` had both condition fields in Fixed mode instead of Expression, so they never matched and every video was routed to manual review; (2) `Parse Caption Result` used `.first()` (execution-wide, not branch-scoped) to fetch original item data, so once a video reached it, its identity got silently overwritten by whatever image was in the same batch; (3) `Mark as Posted` hardcoded its sheet lookup to an image-only upstream node. All three fixed and verified live 19 Sep — see `07-changelog.md`.
...
- [ ] **A shared downstream node can silently assume only one upstream branch ever feeds it.** When two branches (e.g. image vs. video) converge into the same later nodes, anything that references a specific upstream node by name — `$('SomeNode').first()`, `$('SomeNode').item...`, or a hardcoded `{{ $('SomeNode')... }}` expression — will misbehave the moment the *other* branch's item passes through, because that node genuinely never ran for this item. `.first()` in particular is dangerous because it doesn't throw just because "the wrong branch" is active — it throws only if that node never ran *anywhere* in the whole execution, so a try/catch built around it can look correct for months and only misfire once both branches are active in the same run. Prefer `.itemMatching(<index>)` (branch-aware, paired-item-based) over `.first()`, and prefer reading a field off the current item (`$json.field`) over reaching back to a specific named node, whenever a node could be reached by more than one branch. Caught this pattern twice in Flow B on 19 Sep (`Parse Caption Result`, `Mark as Posted`) — worth auditing other shared nodes across the other flows.
