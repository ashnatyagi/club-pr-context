...
- **Steps (high level):**
  ...
  4. **Non-image branch:** `Is Video For Posting?` (If node — checks `source_type == 'video' AND status == 'tagged'`; both conditions must be in **Expression** mode, not Fixed, or they silently compare literal text and never match — see `07-changelog.md`, 2026-09-19).
     - **True (tagged, fast-track video):** `Prepare Video For Posting` (Code) → joins the same posting path as the image branch: `Generate SEO Caption` → `Parse Caption Result` → `Shape Posting Queue Row` → `Append to Posting Queue` → `Mark as Posted`.
     - **False:** `Notify Non-Image Asset` (Telegram) → `Mark Non-Image Reviewed` (update sheet).
...
- **Known issues:** Fixed 2026-09-17 — text overlay was cropping long club+event names, causing false QC fails (see changelog). Fixed 2026-09-19 — `Generate SEO Caption` was on a deprecated Groq model, crashing every execution (see changelog). Fixed 2026-09-19 — video auto-posting is now fully wired up: `Is Video For Posting?` had two condition fields stuck in Fixed mode (never matched), `Parse Caption Result` silently overwrote video identity with whatever image was in the same batch (`.first()` isn't branch-scoped), and `Mark as Posted` hardcoded a lookup to an image-only node. All three fixed, published, and verified live end to end — see `07-changelog.md`.
